...
function TransactionsLine({ transactions }) {
  const deleteTransaction = async ()=>{
    await fetch(`https://wineerpback.herokuapp.com/transactions/`+transactions._id, {
      method: "DELETE",
      ...
      <table className="table-fixed border w-full ">
        ...
        <tbody>
          <tr>
            ...
            <td  className="w-1/12 px-12 border" >{transactions.reference_price}</td>
            <td  className="w-1/12 px-12 border" readOnly >{transactions.total_price}</td>
            ...
            <td  className="w-1/12 px-12 border text-red-700 hover:text-red-500" >
              <button value="6" onClick={(e)=>deleteTransaction(transactions._id)}><Delete /></button>
            </td>
          </tr>
        </tbody>
      </table>
